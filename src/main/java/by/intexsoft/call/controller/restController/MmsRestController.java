...
public class MmsRestController {
    ...
    public void mmsInfo(@RequestBody String data) {
        ...
        List<Mms> mmsList = mmsService.getMmsPeriodTime(type, dataStart, dataEnd);
        fileService.saveToFile(mmsList, type);
        String message = GenerateMessage.createMessage(json.getString("start"), json.getString("end"), mmsList.size());
